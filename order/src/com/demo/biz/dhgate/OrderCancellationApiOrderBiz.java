...
@Service
@Transactional
public class OrderCancellationApiOrderBiz {
	 ...
	 * @return
	 */
	public String autoFetchOrderCancellationOrders(ZhangHao dhAccount) {
	...
	private String fetchOrders(ZhangHao dhAccount,  String startDate, 
		...
		try {
			...
			paramMap.put("statusList", "102009,111000");
		...
		try {
			...
			if (orders != null && !orders.isEmpty()) { // 修改退款
				...
				if (orders.get(0).getJiufen() == null || orders.get(0).getJiufen().equals(0l)) {
					orderTableDao.updateOrderMoney(orderId,orders.get(0).getMoney());
				}
			}
		} catch (Exception e) {
			// TODO: handle exception
