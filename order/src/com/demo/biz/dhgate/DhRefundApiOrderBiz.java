...
@Service
@Transactional
public class DhRefundApiOrderBiz {
	 ...
	 * @return
	 */
	public String autoFetchRefundOrders(ZhangHao dhAccount) {
	...
	private String fetchOrders(ZhangHao dhAccount,  String startDate, 
		...
		try {
			...
			paramMap.put("statusList", "105002,105003,106003");
	...
	private int updateOrder(ZhangHao dhAccount, JSONObject json) {
		try {
			...
			System.out.println("++订单号++"+orderId+"===总退款额===="+totalrefund);
			if (orders != null && !orders.isEmpty()) { // 修改退款
				if (orders.get(0).getJiufen() == null || orders.get(0).getJiufen().equals(0l)) {
					orderTableDao.updateOrderMoney(orderId,totalrefund);
				}
			}
		} catch (Exception e) {
			// TODO: handle exception
