...
@Service
@Transactional
public class DhDisputeTheOrderBiz {
	 ...
	 * @return
	 */
	public String autoFetchDisputesOrders(ZhangHao dhAccount) {
		Date endDate = new Date();
		Date startDate = DateUtils.getAfterDaysDate(endDate, -150); // 取30天内
	...
	private String fetchOrders(ZhangHao dhAccount,  String startDate, 
		...
		try {
			...
			paramMap.put("statusList", "105001,106001");
	...
	private int updateOrder(ZhangHao dhAccount, JSONObject json) {
		...
		if (orders != null && !orders.isEmpty()) { // 修改纠纷

				 ZhangHao zh = zhangHaoDao.get(orders.get(0).getZhanghaoId());
		         LeiMuTable lm = leiMuDao.get(orders.get(0).getLeimuid());
				 orderStatZhBiz.updateStatData(orders.get(0).getTime(), zh); //账号
            	 orderStatLmBiz.updateStatData(orders.get(0).getTime(), lm);//类目
           
            	 List<YeWu1> ss = yewu1dao.getTimes();//查询最久时间
         		if (orders.get(0).getJiufen() == null || orders.get(0).getJiufen().equals(0l)) {
            	 System.out.println("----------------修改纠纷-----------------"+orderId+"+++业务编号+++"+ss.get(0).getUserid());
            	 orderTableDao.updateDisputeStatus(orderId,ff, ss.get(0).getUserid()); 
            	 yewu1dao.updateDisputeStatus(ff, ss.get(0).getUserid());
         		}
		}
		return 1;
	}
