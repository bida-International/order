...
@Service
@Transactional
public class DhApplicationApiOrderBiz {
	 ...
	 * @return
	 */
	public String autoFetchApplicationOrders(ZhangHao dhAccount) {
	...
	private String fetchOrders(ZhangHao dhAccount,  String startDate, 
		...
		try {
			...
			paramMap.put("statusList", "105004");
	...
	private int updateOrder(ZhangHao dhAccount, JSONObject json) {
		try {
			...
			if (orders != null && !orders.isEmpty()) { 
					orderTableDao.updateQxtk(orderId);
			}
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
