...
public class SuccessfulDataJob implements InitializingBean {
...
//		}
		...
		System.out.println("----同步敦煌交易成功订单数据----");
		try {
			...
			for (ZhangHao zhangHao : zhangHaos) {
				if (zhangHao.getAccount_type().equals(DhCommonApiBiz.ACCOUNT_TYPE)) { // 敦煌账号
					// 同步订单数据
					String result = dhSuccessfulApiOrderBiz.autoFetchSuccessfulOrders(zhangHao);
