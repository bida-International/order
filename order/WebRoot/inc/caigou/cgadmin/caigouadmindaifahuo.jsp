...
function as(){
 ...
 function mysumaitong(){
	    ...
	    var lujing = "caigouadmin!upsumaitong.do?pageNumber="+${pageNumber}+"&pager.offset="+${pageNumber*10-10};
  ...
  function zhupdate(){
	...
	if(flag<1){
		...
		if(bool){
		  ...
		  //location.href="caigouadmin!myupzhanghao.do?pageNumber="+${pageNumber}+"&pager.offset="+${pageNumber*10-10}+"&bulletinId="+bulletinId+"&seluserid="+sel;
		}else{
			return false;
		}
	}
	   // var lujing = "caigouadmin!myupzhanghao.do?pageNumber="+${pageNumber}+"&pager.offset="+${pageNumber*10-10};
 }
  function quan()
 {
  ...
  function tiaozhuan(){
	...
	location.href = "caigouadmin!getDaiFaHuo.do?pager.offset="+(pageNumber*10-10)+"&pageNumber="+pageNumber;
  }
...
<m:frame>
<form action="caigouadmin!getDaiFaHuo.do"  onsubmit="return as()"  method="post" name="forms" id="forms">
<table  class="datagrid2" id="list"><tr><td><input type="button" style="cursor:pointer" value="速卖通返回给管理员" onclick="mysumaitong()"/></td></tr></table>
  <table  class="datagrid2" id="list" border="1" width="1100" cellspacing="0" style="float:left">
    <tr>
    ...
    <tr>
    	<td colspan="18" align="left">
	  	...
	  	物品: <input type="text" name="wuping" size="8" id="wuping" value="${param.wuping}"/><br/>
    ...
    <td width="34" height="25">
     ...
     <td width="54"><span class="STYLE2">编码</span></td>
      ...
      <td width="100"><span class="STYLE2">订单状态</span></td>
	     ...
	     <td>${sd.bianma}</td>
	         ...
	         <s:if test="#sd.leimuid != null">
	   		 ...
	   		 <td><textarea rows="5" cols="10">${sd.remark}</textarea></td> 
	   		  <td><a href="caigouadmin!upadmindaifahuo.do?ordertable.id=${sd.id}&pageNumber=${pageNumber}&pager.offset=${pageNumber*10-10}">修改</a></td>
		     <td><textarea rows="5" cols="10">${sd.danhao}</textarea></td>
		      <td>${sd.time}</td>	    
		       <td>
		       <s:if test="(#sd.daifahuo == 1 && (#sd.guoneidanhao == null || #sd.guoneidanhao == ''))">
		       		仓库单号为空
		       	</s:if>
		       	<s:if test="(#sd.daifahuo == 1 && (#sd.guoneidanhao != null && #sd.guoneidanhao != ''))">
		       		仓库单号不为空
		       	</s:if>
		       	<s:if test="(#sd.daifahuo == 2)">
		       		仓库待放区
		       	</s:if>
