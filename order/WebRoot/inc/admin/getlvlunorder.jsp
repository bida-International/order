...
<m:frame>
	<script type="text/javascript">
	 ...
	 <form action="admin!getlvlunorder.do"  name="forms" id="forms" method="post" onsubmit="return as()">

  <table border="1"  cellspacing="0" cellpadding="0" align="center" width="1500px" cellspacing="0">
    <tr>
      <td colspan="21" align="center"><strong>利润率查询</strong>
    	...
    	类目查询：
	    	...
	    	利润率:
	    	 <select name="tpm" id="tpm">
	    		<option value="">-请选择-</option>
		    	<option value="0.1">-大于0.1-</option>
		    	<option value="0.2">-大于0.2-</option>
		    	<option value="0.3">-大于0.3-</option>
      ...
      <td><input name="chkItems" value="${i.id}" onclick="chkItems_click(this)" type="checkbox"  id="chk_${i.id}" readonly="readonly"  style="cursor:pointer"/></td>
	    ...
	    <td>${i.country}</td>	    
	     ...
	     <td>
	      ...
	      <td><fmt:formatNumber value="${(i.money*i.huilv-(i.huokuan+i.yunfei+i.tuikuan*i.huilv))/(i.money*i.huilv)}"  pattern="0.00"></fmt:formatNumber></td>
	     <td>${i.wuping}</td>
	     <td><s:property value="getZhangHaoId(#i.zhanghaoId)"/></td>
	     <td><textarea rows="5" cols="10">${i.remark}</textarea></td>
	   	 <td><s:property value="getUserId(#i.caigouyuan)"/></td>	
     </tr>    
   </s:iterator>
 <tr>
   	<td colspan="21">
  		...
  		<a href=""> 利润率：<s:property value="getTheProfitMargin(orderId, time, time1, zhanghao,danhao,sumaitong,bianma,category,country)"/></a>
   		<br>
   	</td>
   </tr> 
    <tr> 
